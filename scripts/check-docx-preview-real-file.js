...
const APP_PORT = 4567;
const CDP_PORT = 9261;
const REAL_FILE = process.env.ARCA_DOCX_REAL_FILE || 'D:\\新人\\财务\\财务实习生考核_实操试题_参考答案.docx';
...
function findBrowser() {
  ...
  if (!hit) throw new Error('Chrome/Edge not found for real docx preview check');
  return hit;
}
...
async function main() {
  if (!fs.existsSync(REAL_FILE)) throw new Error(`real docx fixture missing: ${REAL_FILE}`);
  const stat = fs.statSync(REAL_FILE);
  const profileDir = path.join(os.tmpdir(), `arca-docx-real-file-profile-${Date.now()}`);
  ...
  try {
    ...
    for (let i = 0; i < 60; i += 1) {
      const ready = await client.send('Runtime.evaluate', {
        returnByValue: true,
        expression: `document.readyState === 'complete' && typeof openDocxPreview === 'function'`,
      });
    ...
    const result = await client.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        return {
          ...
          hasSave: !!document.querySelector('#docx-save'),
          hasSystemOpen: !!document.querySelector('#docx-sys'),
          hostChildCount: host ? host.childElementCount : 0,
        };
      })()`,
    });

    if (result.exceptionDetails) throw new Error(`Chrome real docx evaluation failed: ${JSON.stringify(result.exceptionDetails)}`);
    const value = result.result && result.result.value;
    if (!value) throw new Error(`No real docx preview result returned: ${JSON.stringify(result)}`);
    if (/Word 预览失败|Cannot read properties|undefined|relationships/.test(value.text)) throw new Error(`real docx preview failed or leaked raw error: ${JSON.stringify(value)}`);
    if (!value.hasSave || !value.hasSystemOpen || value.hostChildCount < 1) throw new Error(`real docx preview controls missing: ${JSON.stringify(value)}`);
    console.log('docx-preview-real-file ok');
