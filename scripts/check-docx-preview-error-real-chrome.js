...
function findBrowser() {
  ...
  if (!hit) throw new Error('Chrome/Edge not found for docx preview error check');
  return hit;
}
...
async function main() {
  const profileDir = path.join(os.tmpdir(), `arca-docx-preview-error-profile-${Date.now()}`);
  ...
  try {
    ...
    for (let i = 0; i < 60; i += 1) {
      const ready = await client.send('Runtime.evaluate', {
        returnByValue: true,
        expression: `document.readyState === 'complete' && typeof openDocxPreview === 'function' && typeof friendlyDocxPreviewError === 'function'`,
      });
    ...
    const result = await client.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        window.FanboxDocx = {
          mount() {
            throw new TypeError("Cannot read properties of undefined (reading 'relationships')");
          },
        };
        ...
        return {
          ...
          hasFallback: !!document.querySelector('#docx-fallback'),
        };
      })()`,
    });

    if (result.exceptionDetails) throw new Error(`Chrome docx preview evaluation failed: ${JSON.stringify(result.exceptionDetails)}`);
    const value = result.result && result.result.value;
    if (!value) throw new Error(`No docx preview result returned: ${JSON.stringify(result)}`);
    if (!value.hasFallback) throw new Error(`docx fallback button missing: ${JSON.stringify(value)}`);
    if (!value.text.includes('文档结构异常，建议用系统应用打开')) throw new Error(`friendly docx error missing: ${JSON.stringify(value)}`);
    if (/Cannot read properties|relationships|undefined/.test(value.text)) throw new Error(`raw JS error leaked to user: ${JSON.stringify(value)}`);
    console.log('docx-preview-error-real-chrome ok');
