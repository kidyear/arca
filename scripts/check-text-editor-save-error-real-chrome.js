...
function findBrowser() {
  ...
  if (!hit) throw new Error('Chrome/Edge not found for text editor save error check');
  return hit;
}
...
async function main() {
  const profileDir = path.join(os.tmpdir(), `arca-text-editor-save-error-profile-${Date.now()}`);
    ...
    const result = await client.send('Runtime.evaluate', {
      ...
      expression: `new Promise(async (resolve) => {
        ...
        window.fetch = async (url, opts) => {
          const s = String(url);
          if (s.includes('/api/read')) {
            return new Response(JSON.stringify({ content: 'old text', ext: '.txt', mtime: 1 }), { headers: { 'Content-Type': 'application/json' } });
          }
          if (s.includes('/api/write')) {
            return new Response(JSON.stringify({ ok: false }), { headers: { 'Content-Type': 'application/json' } });
          }
          return originalFetch(url, opts);
        };
        await enterEditMode({ path: 'D:\\\\保存失败测试\\\\note.txt', name: 'note.txt', isDir: false, kind: 'text', size: 8, mtime: 1, btime: 1 });
        ...
        setTimeout(() => resolve({
          ...
          editorExists: !!document.querySelector('#ed-host'),
        }), 180);
      })`,
    });

    if (result.exceptionDetails) throw new Error(`Chrome text editor save error evaluation failed: ${JSON.stringify(result.exceptionDetails)}`);
    const value = result.result && result.result.value;
    if (!value) throw new Error(`No text editor save error result returned: ${JSON.stringify(result)}`);
    if (!value.editorExists) throw new Error(`text editor fixture did not render: ${JSON.stringify(value)}`);
    if (!value.toast.includes('保存失败：未知错误')) {
      throw new Error(`text editor save failure should fall back to unknown error: ${JSON.stringify(value)}`);
    }
    console.log('text-editor-save-error-real-chrome ok');
