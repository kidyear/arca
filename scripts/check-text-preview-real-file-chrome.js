...
function findBrowser() {
  ...
  if (!hit) throw new Error('Chrome/Edge not found for real text preview check');
  return hit;
}
...
async function main() {
  ...
  const profileDir = path.join(os.tmpdir(), `arca-text-real-file-profile-${Date.now()}`);
  ...
  try {
    ...
    for (let i = 0; i < 60; i += 1) {
      const ready = await client.send('Runtime.evaluate', {
        returnByValue: true,
        expression: `document.readyState === 'complete' && typeof openPreview === 'function'`,
      });
    ...
    const result = await client.send('Runtime.evaluate', {
      ...
      expression: `(async () => {
        ...
        await openPreview({
          path: ${JSON.stringify(REAL_FILE)},
          name: ${JSON.stringify(path.basename(REAL_FILE))},
          kind: 'text',
          isDir: false,
          size: ${stat.size},
          mtime: ${stat.mtimeMs},
        ...
        return {
          ...
          meta: [...document.querySelectorAll('#preview-body .preview-meta span')].map((el) => el.textContent.trim()),
          codeText: document.querySelector('#preview-body pre code')?.textContent || '',
        };
      })()`,
    });

    if (result.exceptionDetails) throw new Error(`Chrome real text evaluation failed: ${JSON.stringify(result.exceptionDetails)}`);
    const value = result.result && result.result.value;
    if (!value) throw new Error(`No real text preview result returned: ${JSON.stringify(result)}`);
    if (!value.title.includes('重要说明.txt')) throw new Error(`preview title missing file name: ${JSON.stringify(value)}`);
    if (!value.meta.includes('GBK')) throw new Error(`preview metadata should show GBK encoding: ${JSON.stringify(value)}`);
    for (const phrase of ['该参考设计版本', 'Change list', 'PCB设计文件']) {
      if (!value.codeText.includes(phrase)) throw new Error(`text preview missing phrase ${phrase}: ${JSON.stringify(value)}`);
    }
    if (/\uFFFD|锟斤拷|���/.test(value.text)) throw new Error(`text preview still looks garbled: ${JSON.stringify(value)}`);
    console.log('text-preview-real-file-chrome ok');
