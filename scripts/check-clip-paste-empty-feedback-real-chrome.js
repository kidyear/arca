...
function findBrowser() {
  ...
  if (!hit) throw new Error('Chrome/Edge not found for empty paste feedback check');
  return hit;
}
...
async function main() {
  const profileDir = path.join(os.tmpdir(), `arca-empty-paste-profile-${Date.now()}`);
  ...
  try {
    ...
    for (let i = 0; i < 60; i += 1) {
      const ready = await client.send('Runtime.evaluate', {
        returnByValue: true,
        expression: `document.readyState === 'complete' && typeof clipPaste === 'function' && typeof bindEvents === 'function'`,
      });
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            window.fetch = async (url, opts) => {
              if (String(url).includes('/api/copy-in')) {
            ...
            clipPaste('D:\\\\目标目录').then(() => {
              setTimeout(() => resolve({
                beforeToast,
                afterToast,
                readErrorToast,
                pasteFailureToast: document.querySelector('#toast')?.textContent || '',
    ...
    if (result.exceptionDetails) throw new Error(`Chrome empty paste evaluation failed: ${JSON.stringify(result.exceptionDetails)}`);
    const value = result.result && result.result.value;
    if (!value) throw new Error(`No empty paste result returned: ${JSON.stringify(result)}`);
    if (!value.afterToast.includes('剪贴板中没有可粘贴的文件')) throw new Error(`empty file paste should show a clear toast: ${JSON.stringify(value)}`);
    if (!value.readErrorToast.includes('读取剪贴板失败：clipboard locked')) throw new Error(`clipboard read failure should show a distinct toast: ${JSON.stringify(value)}`);
    if (!value.pasteFailureToast.includes('粘贴失败：access denied')) throw new Error(`copy paste failure should keep operation context and backend reason: ${JSON.stringify(value)}`);
    if (value.fileClip !== null) throw new Error(`empty file paste should not create internal clipboard state: ${JSON.stringify(value)}`);
    console.log('clip-paste-empty-feedback-real-chrome ok');
