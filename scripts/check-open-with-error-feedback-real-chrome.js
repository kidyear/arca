...
function findBrowser() {
  ...
  if (!hit) throw new Error('Chrome/Edge not found for openWith error feedback check');
  return hit;
}
...
async function main() {
  const profileDir = path.join(os.tmpdir(), `arca-open-with-error-profile-${Date.now()}`);
  ...
  try {
    ...
    for (let i = 0; i < 60; i += 1) {
      const ready = await client.send('Runtime.evaluate', {
        returnByValue: true,
        expression: `document.readyState === 'complete' && typeof openWith === 'function'`,
      });
    ...
    const result = await client.send('Runtime.evaluate', {
      ...
      expression: `new Promise((resolve) => {
        ...
        window.fetch = async (url, opts) => {
          if (String(url).includes('/api/open')) throw new Error('network down');
          return originalFetch(url, opts);
        };
        openWith('D:\\\\文件\\\\demo.docx', 'reveal');
        setTimeout(() => resolve({
    ...
    if (result.exceptionDetails) throw new Error(`Chrome openWith error evaluation failed: ${JSON.stringify(result.exceptionDetails)}`);
    const value = result.result && result.result.value;
    if (!value) throw new Error(`No openWith error result returned: ${JSON.stringify(result)}`);
    if (!value.toast.includes('打开失败：network down')) {
      throw new Error(`openWith request failure should show network reason: ${JSON.stringify(value)}`);
    }
    console.log('open-with-error-feedback-real-chrome ok');
