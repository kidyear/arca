...
function findBrowser() {
  ...
  if (!hit) throw new Error('Chrome/Edge not found for trash selection error check');
  return hit;
}
...
async function main() {
  const profileDir = path.join(os.tmpdir(), `arca-trash-selection-error-profile-${Date.now()}`);
  ...
  try {
    ...
    for (let i = 0; i < 60; i += 1) {
      const ready = await client.send('Runtime.evaluate', {
        returnByValue: true,
        expression: `document.readyState === 'complete' && typeof trashSelection === 'function' && typeof renderFiles === 'function'`,
      });
    ...
    const result = await client.send('Runtime.evaluate', {
      ...
      expression: `new Promise((resolve) => {
        ...
        const left = { path: 'D:\\\\删除失败测试\\\\ok.txt', name: 'ok.txt', isDir: false, kind: 'text', size: 1, mtime: Date.now(), btime: Date.now() };
        const right = { path: 'D:\\\\删除失败测试\\\\locked.txt', name: 'locked.txt', isDir: false, kind: 'text', size: 1, mtime: Date.now(), btime: Date.now() };
        ...
        state.visible = [];
        state.selected = right.path;
        state.selectionAnchor = right.path;
        state.multiSel = new Set([left.path, right.path]);
        renderFiles();
        window.confirmDialog = async () => true;
        const originalFetch = window.fetch.bind(window);
        window.fetch = async (url, opts) => {
          if (String(url).includes('/api/trash-undoable')) {
            const body = JSON.parse(opts?.body || '{}');
            if (String(body.path || '').includes('locked.txt')) {
              return new Response(JSON.stringify({ ok: false, error: 'access denied' }), { status: 200, headers: { 'Content-Type': 'application/json' } });
            }
        ...
        trashSelection().then(() => setTimeout(() => resolve({
          toast: document.querySelector('#toast')?.textContent || '',
          undoType: state.undoStack[state.undoStack.length - 1]?.type || '',
          undoCount: state.undoStack[state.undoStack.length - 1]?.items?.length || 0,
        }), 120));
      })`,
    });

    if (result.exceptionDetails) throw new Error(`Chrome trash selection error evaluation failed: ${JSON.stringify(result.exceptionDetails)}`);
    const value = result.result && result.result.value;
    if (!value) throw new Error(`No trash selection error result returned: ${JSON.stringify(result)}`);
    if (!value.toast.includes('完成，1 项删除失败：access denied')) throw new Error(`multi trash failure should show backend reason: ${JSON.stringify(value)}`);
    if (value.undoType !== 'trash' || value.undoCount !== 1) throw new Error(`successful trash items should remain undoable: ${JSON.stringify(value)}`);
    console.log('trash-selection-error-real-chrome ok');
