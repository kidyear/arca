...
function findBrowser() {
  ...
  if (!hit) throw new Error('Chrome/Edge not found for new window error feedback check');
  return hit;
}
...
async function main() {
  const profileDir = path.join(os.tmpdir(), `arca-new-window-error-profile-${Date.now()}`);
  ...
  try {
    ...
    for (let i = 0; i < 60; i += 1) {
      const ready = await client.send('Runtime.evaluate', {
        returnByValue: true,
        expression: `document.readyState === 'complete' && typeof openNewWindow === 'function' && typeof closeCurrentWindow === 'function'`,
      });
    ...
    const result = await client.send('Runtime.evaluate', {
      ...
      expression: `new Promise(async (resolve) => {
        ...
        await openNewWindow('D:\\\\窗口失败测试');
        const openToast = document.querySelector('#toast')?.textContent || '';
        await closeCurrentWindow();
        setTimeout(() => resolve({
          openToast,
          closeToast: document.querySelector('#toast')?.textContent || '',
        }), 80);
      })`,
    });

    if (result.exceptionDetails) throw new Error(`Chrome new window error evaluation failed: ${JSON.stringify(result.exceptionDetails)}`);
    const value = result.result && result.result.value;
    if (!value) throw new Error(`No new window error result returned: ${JSON.stringify(result)}`);
    if (!value.openToast.includes('新窗口打开失败：未知错误')) {
      throw new Error(`new window failure should fall back to unknown error: ${JSON.stringify(value)}`);
    }
    if (!value.closeToast.includes('关闭窗口失败：未知错误')) {
      throw new Error(`close window failure should fall back to unknown error: ${JSON.stringify(value)}`);
    }
    console.log('new-window-error-real-chrome ok');
