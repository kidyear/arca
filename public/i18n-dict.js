...
window.FANBOX_DICT = {
  ...
  '复制文件（系统文件管理器可粘贴）': 'Copy file (paste in system file manager)',
  ...
  '已放进对话输入框，补一句要求再发送': 'Added to the chat input — add your request, then send',
  '已发到终端，确认后再回车': 'Sent to terminal — review it, then press Enter',
  ...
  '已保存': 'Saved',
  '保存失败': 'Save failed',
  'AI 设置暂时无法连接，请稍后重试': 'AI settings cannot connect right now. Try again later',
  'AI 认证失败，请检查 API key': 'AI authentication failed. Check the API key',
  'AI 服务限流或额度不足，请稍后重试': 'AI service is rate limited or out of quota. Try again later',
  '模型不可用，请检查模型名称': 'Model unavailable. Check the model name',
  'AI 设置返回异常，请稍后重试': 'AI settings returned an unexpected error. Try again later',
  ...
  // ---------- 操作提示（toast）----------
  '打开失败': 'Open failed',
  '已在文件管理器中显示': 'Revealed in file manager',
  ...
  '已复制文件，可在系统文件管理器里粘贴': 'File copied — paste it in the system file manager',
  ...
  '内嵌终端不可用，请使用桌面版或检查终端组件': 'Embedded terminal unavailable. Use the desktop app or check terminal components',
...
window.FANBOX_DICT_RULES = [
  ...
  [/^新窗口打开失败：([\s\S]*)$/, (m) => `Open in new window failed: ${m[1]}`],
  [/^关闭窗口失败：([\s\S]*)$/, (m) => `Close window failed: ${m[1]}`],
  ...
  [/^读取 AI 配置失败：([\s\S]*)$/, (m) => `Failed to read AI settings: ${m[1]}`],
  [/^拉取失败：([\s\S]*)$/, (m) => `Fetch failed: ${m[1]}`],
  ...
  [/^终端重开失败：([\s\S]*)$/, (m) => `Terminal failed to restart: ${m[1]}`],
  [/^重开失败：([\s\S]*)$/, (m) => `Restart failed: ${m[1]}`],
  [/^复制失败：([\s\S]*)$/, (m) => `Copy failed: ${m[1]}`],
  [/^复制路径失败，路径：([\s\S]+)：([\s\S]*)$/, (m) => `Copy path failed: ${m[2]}. Path: ${m[1]}`],
  [/^复制路径失败，路径：([\s\S]+)$/, (m) => `Copy path failed. Path: ${m[1]}`],
  // 文件操作
  [/^已另存为 (.+)$/, (m) => `Saved as ${m[1]}`],
  [/^快捷方式目标不存在：([\s\S]*)$/, (m) => `Shortcut target is missing: ${m[1]}`],
  [/^已打开快捷方式目标$/, () => 'Opened shortcut target'],
  [/^粘贴快捷方式到「(.+)」$/, (m) => `Paste shortcut into "${m[1]}"`],
  [/^已粘贴 (\d+) 个快捷方式$/, (m) => `Pasted ${m[1]} shortcuts`],
  [/^已粘贴 (\d+) 个快捷方式，(\d+) 项失败$/, (m) => `Pasted ${m[1]} shortcuts, ${m[2]} failed`],
